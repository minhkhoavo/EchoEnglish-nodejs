    ...
    try {
      ...
      if (
        ...
        for (const seg of (transformed as Record<string, unknown>)
          ...
          for (const w of (seg.words as Array<Record<string, unknown>>) || []) {
            ...
            if (st) (w as Record<string, unknown>).isStressed = !!st.isStressed;
          }
        }
      }
      ...
      try {
        const transcriptText =
          ...
          Array.isArray((finalPayload as Record<string, unknown>).segments)
            ? (
                (finalPayload as Record<string, unknown>).segments as Array<
...
class SpeechController {
  ...
  async listRecordings(req: Request, res: Response) {
    const userId =
      (req.user as { id?: string })?.id || (req as { userId?: string }).userId;
