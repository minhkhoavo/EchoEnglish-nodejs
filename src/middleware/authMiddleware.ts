...
export function hasAuthority(...roles: string[]) {
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!hasRole) {
            return res
                .status(403)
                .json(new ApiResponse('Insufficient permission'));
        }
...
export function isOwn(model: Model<unknown>, idParam: string = 'id') {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        if ((doc as unknown as Record<string, unknown>).createBy !== userId) {
