...
class AIScoringService {
    private getTemplateNameForType(type: IQuestionContext['questionType']): string {
    ...
    public async scoreRecording(recordingId: string | Types.ObjectId, context: IQuestionContext): Promise<any> {
        const summary: any = await RecordingService.getRecordingSummary(recordingId);
        if (!summary) {
        ...
        try {
            ...
            throw new Error("AI model failed to process the request or return valid JSON.");
        }
    }
}
