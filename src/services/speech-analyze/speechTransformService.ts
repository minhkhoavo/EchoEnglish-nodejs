class SpeechTransformService {
  private extractErrors(
    ...
    const errorType = (pronAssessment as Record<string, unknown>)?.ErrorType;
    ...
    switch (errorType) {
      ...
      case 'UnexpectedBreak': {
        ...
        const feedback = assessment?.Feedback as Record<string, unknown>;
      ...
      case 'MissingBreak': {
        ...
        const prosody = feedback?.Prosody as Record<string, unknown>;
        const breakData = prosody?.Break as Record<string, unknown>;
        const missingBreak = breakData?.MissingBreak as Record<string, unknown>;
      ...
      case 'Monotone': {
        ...
        const feedback = assessment?.Feedback as Record<string, unknown>;
        const prosody = feedback?.Prosody as Record<string, unknown>;
        const intonation = prosody?.Intonation as Record<string, unknown>;
        const monotone = intonation?.Monotone as Record<string, unknown>;
        const monotoneConfidence = monotone?.SyllablePitchDeltaConfidence;
  ...
  private transformSegmentToWords(segmentData: unknown) {
    ...
    if (!segment?.NBest || !Array.isArray(segment.NBest) || !segment.NBest[0])
      return [];
    ...
    const segmentConfidence = ((firstResult.Confidence as number) || 0) * 100;
    ...
    return words.map((wordData: unknown, idx: number) => {
      ...
      const prevWord =
        idx > 0 ? (words[idx - 1] as Record<string, unknown>)?.Word : undefined;
      const isDuplicated =
      ...
      const phonemes = Array.isArray(word.Phonemes)
        ? word.Phonemes.map((phonemeData: unknown) => {
            ...
            const pronAssessment = phoneme.PronunciationAssessment as Record<
              string,
              unknown
            >;
            const accuracyScore =
            ...
            return {
              ...
              offset:
                ((phoneme?.Offset as number) || 0) / TICKS_PER_MILLISECOND,
              duration:
                ((phoneme?.Duration as number) || 0) / TICKS_PER_MILLISECOND,
      ...
      const syllables = Array.isArray(word.Syllables)
        ? word.Syllables.map((syllableData: unknown) => {
            ...
            const pronAssessment = syllable.PronunciationAssessment as Record<
              string,
              unknown
            >;
            return {
              ...
              offset:
                ((syllable?.Offset as number) || 0) / TICKS_PER_MILLISECOND,
              duration:
                ((syllable?.Duration as number) || 0) / TICKS_PER_MILLISECOND,
            };
          })
        : [];
      ...
      const actualPronunciation = `/${phonemes
        ...
        .map((phoneme: Record<string, unknown>) => phoneme.actualPhoneme)
        .join('')}/`;
      const errors = this.extractErrors(word.PronunciationAssessment);
      return {
        word: word.Word,
        accuracy: (word?.PronunciationAssessment as Record<string, unknown>)
          ?.AccuracyScore,
        offset: ((word?.Offset as number) || 0) / TICKS_PER_MILLISECOND,
        duration: ((word?.Duration as number) || 0) / TICKS_PER_MILLISECOND,
        phonemes,
        syllables,
        errors,
        isStressed: false,
  ...
  createTranscriptData(
    ...
    const segments = azureResponseArray
      .map((segmentData, index) => {
        ...
        const overallPronunciationScore = NBest?.[0]?.PronunciationAssessment
          ? ((NBest[0].PronunciationAssessment as Record<string, unknown>)
    ...
    const lastSegment = segments[segments.length - 1] as Record<
      ...
      unknown
    >;
    const totalDuration = segments.length ? (lastSegment.endTime as number) : 0;
      ...
      overall: {
        AccuracyScore: (() => {
          const firstResponse = azureResponseArray?.[0] as Record<
            ...
            unknown
          >;
          const NBest = firstResponse?.NBest as Array<Record<string, unknown>>;
