  ...
  analyze({
    ...
    for (const seg of ((transformed as Record<string, unknown>)
      ...
      for (const w of (seg.words as Array<Record<string, unknown>>) || []) {
        ...
        const endSec =
          (((w.offset as number) || 0) + ((w.duration as number) || 0)) / 1000;
        allWords.push({
    ...
    let fluency: FluencyAnalysis = {
      ...
      points: (
        ...
        const durationSec = Math.max(
          0.001,
          ((segment.endTime as number) - (segment.startTime as number)) / 1000
        );
        const wpm =
          (((segment.words as Array<unknown>)?.length || 0) / durationSec) * 60;
        return {
          time: Number(((segment.endTime as number) / 1000).toFixed(2)),
