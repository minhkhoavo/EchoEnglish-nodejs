...
class PronunciationSummaryService {
  summarize(azureResponseArray: any[]): {
    chartData: ChartItem[];
      ...
      for (const word of words) {
        ...
        for (const ph of phonemes) {
          ...
          if (acc < ERROR_THRESHOLD) {
            const actual =
              ph?.PronunciationAssessment?.NBestPhonemes?.[0]?.Phoneme ||
              'omitted';
    ...
    const topMistakes: TopMistake[] = topSounds.map((item) => {
      ...
      const resources = Array.isArray((resourceIndex as any)[key])
        ? (resourceIndex as any)[key].slice(0, 3)
        : [];
