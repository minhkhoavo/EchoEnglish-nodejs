        ...
        switch (errorType) {
            ...
            case 'UnexpectedBreak': {
                ...
                if (typeof unexpectedBreakConfidence === 'number')
                    errors.push({ type: 'unexpected_break', confidence: Math.round(unexpectedBreakConfidence * 100) });
                break;
            }
            case 'MissingBreak': {
                ...
                if (typeof missingBreakConfidence === 'number')
                    errors.push({ type: 'missing_break', confidence: Math.round(missingBreakConfidence * 100) });
                break;
            }
            case 'Monotone': {
                const monotoneConfidence =
                    pronAssessment?.Feedback?.Prosody?.Intonation?.Monotone?.SyllablePitchDeltaConfidence;
                if (typeof monotoneConfidence === 'number')
                    errors.push({ type: 'monotone', confidence: Math.round((1 - monotoneConfidence) * 100) });
                break;
            }
            default:
                break;
