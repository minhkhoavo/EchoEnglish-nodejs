    ...
    private extractErrors(pronAssessment: any): Array<Record<string, any>> {
        ...
        switch (errorType) {
            ...
            case 'UnexpectedBreak': {
                const unexpectedBreakConfidence = pronAssessment?.Feedback?.Prosody?.Break?.UnexpectedBreak?.Confidence;
                if (typeof unexpectedBreakConfidence === 'number') errors.push({ type: 'unexpected_break', confidence: Math.round(unexpectedBreakConfidence * 100) });
                break;
            }
            case 'MissingBreak': {
                const missingBreakConfidence = pronAssessment?.Feedback?.Prosody?.Break?.MissingBreak?.Confidence;
                if (typeof missingBreakConfidence === 'number') errors.push({ type: 'missing_break', confidence: Math.round(missingBreakConfidence * 100) });
                break;
            }
            case 'Monotone': {
                const monotoneConfidence = pronAssessment?.Feedback?.Prosody?.Intonation?.Monotone?.SyllablePitchDeltaConfidence;
                if (typeof monotoneConfidence === 'number') errors.push({ type: 'monotone', confidence: Math.round((1 - monotoneConfidence) * 100) });
                break;
            }
            default:
                break;
    ...
    createTranscriptData(azureResponseArray: any[], audioUrl: string = ''): any {
        const segments = azureResponseArray
            .map((segmentData, index) => {
                ...
                const overallPronunciationScore = segmentData?.NBest?.[0]?.PronunciationAssessment?.PronScore || 0;
                return {
                    id: `segment-${index + 1}`,
        ...
        return {
            ...
            advancedFeatures: {
                ...
                CompletenessScore: azureResponseArray?.[0]?.NBest?.[0]?.PronunciationAssessment?.CompletenessScore || 0,
                PronScore: azureResponseArray?.[0]?.NBest?.[0]?.PronunciationAssessment?.PronScore || 0,
            },
        };
    }
}
