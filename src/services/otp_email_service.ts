    ...
    return Otp.findOne({ email: normalizedEmail, otp: otpCode, purpose })
      ...
      .then((otpDoc) => {
        ...
        if (otpDoc.expiryTime < new Date()) {
          return Otp.deleteOne({ _id: otpDoc._id }).exec().then(() => false);
        }

        return Otp.deleteMany({ email: normalizedEmail, purpose }).exec().then(() => {
          if (purpose === OtpPurpose.REGISTER) {
            return User.findOne({ email: normalizedEmail }).exec().then((user) => {
